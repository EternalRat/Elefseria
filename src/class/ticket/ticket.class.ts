...
export class Ticket {
    ...
    constructor(
        channel: TextChannel,
        owner: string,
        permissions: Array<OverwriteResolvable>,
    ) {
        this.id = channel.id;
        ...
        (async () => {
            ...
            if (!this.TicketHandler) {
                console.info('Ticket not found, creating new ticket');
                this.messageEmbed = await channel.send(
                    this.optionsTicketCommandEmbed(this.isClosed),
                );
                this.TicketHandler = await TicketHandler.createTicket(
                    channel.id,
                    owner,
                    permissions,
                    this.messageEmbed.id,
                );
                this.TicketHandler?.addTicketToGuild(channel.guild.id);
                this.TicketHandler?.addTicketToUser(owner);
                return;
            } else {
                this.messageEmbed = await channel.messages.fetch(
                    this.TicketHandler.embedMessage,
                );
            }
            console.info('Ticket found');
            console.info(this.TicketHandler);
        })();
    }
    ...
    public async closeTicket(
        ...
        console.info(this.messageEmbed);
        if (this.messageEmbed) {
            await this.messageEmbed.delete();
            this.messageEmbed = await channel.send(
                this.optionsTicketCommandEmbed(this.isClosed),
    ...
    public async deleteTicket(
        ...
        const sendMessage = await channel.send({
            content:
                "'You can cancel the deletion of the ticket by clicking on the button below'",
            components: [row],
        });
            ...
            try {
                ...
                if (!this.isBeingDeleted) {
                    await sendMessage.edit({
                        content:
                            'Delete canceled (This message will deleted soon)',
                        components: [],
                    });
                ...
                if (i > 0 && this.isBeingDeleted)
                    await interaction.editReply(
                        `Ticket will be deleted in ${i} seconds`,
                    );
                i--;
                if (i < 0 && this.isBeingDeleted) {
    ...
    public async buildTranscript(guildId: string, client: DiscordClient) {
        const channel = client.channels.cache.get(this.id) as TextChannel;
        if (!channel) {
            return;
        }
