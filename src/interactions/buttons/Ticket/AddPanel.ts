...
export class AddPanelButtonInteraction extends BaseButtonInteraction {
    ...
    async execute(
        ...
        const lastPanel = await ticketHandler.createIfLastPanelActive(interaction.guildId!, await ticketHandler.getLastPanelCreated(
        ...
        if (page === 6) {
            ...
            return;
        } else if (AddPanelButtonInteraction.pagesBuilder[page].modals) {
            ...
            const btns = buildButtons(AddPanelButtonInteraction.pagesBuilder[page].components);
            const rowRole =
